...
def MarvellousDataShieldStart(data):
    ...
    ext_file = "extensions.txt" if os.path.exists("extensions.txt") else None
    ...
    try:
        ...
        ignore_extensions = ignore_ext(ext_file)
        files = backup_files(data, BackupName, ignore_extensions)

        zip_file = create_zip(BackupName)
        save_backup_history(zip_file, len(files))
    ...
    try:
        if len(sys.argv) == 2:
            ...
            if arvar == "--h" or arvar == "--H":
                ...
                log_info("6 : Display backup history")
                ...
                log_info("")
                log_info("Display backup history:")
                log_info("python Script.py --history")

            elif arvar == "--history" or arvar == "--HISTORY":
                display_backup_history()
                log_info("Backup history displayed")
